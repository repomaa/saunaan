...
<div class="p-8">
  <h1 class="text-4xl mb-8">Saunaan</h1>
    ...
    <ul class="flex gap-8 mb-8">
      {#each bestAppointments as { from, to, count, url }}
        <li class="flex flex-col gap-3 p-3 border rounded">
          ...
          <a href={url} class="text-blue-500">Varaa</a>
        </li>
      {/each}
    </ul>
  {/if}
  ...
  <div class="flex justify-between mb-8">
    <input type="text" bind:value={name} placeholder="Nimi" class="p-2 border rounded" />
    <button type="button" class="bg-green-500 text-white font-semibold p-2 rounded" on:click={save}
      >Tallenna</button
    >
  </div>

  <div class="flex items-stretch gap-8 overflow-x-auto">
    <ul class="flex flex-col justify-end items-end gap-3 sticky left-0 bg-white">
      <li class="h-9">{name}</li>
    ...
    <div class="flex flex-col gap-3">
      ...
      <ul class="flex gap-8">
        {#each sortedAppointments as { id }}
          <li class="w-max">
            <button type="button" on:click={() => toggleVote(id)} class="border rounded w-9 h-9">
              <Vote type={votes[id]} />
            </button>
          </li>
        {/each}
